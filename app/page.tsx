...
const team = [
  { name: "Maxamed A. Xasan", role: "Nootaayaha Guud", experience: "12+ sano oo khibrad sharci iyo nootaayo ah" },
  { name: "Hodan M. Cali", role: "Sarkaalka Sare ee Sharciga", experience: "8+ sano oo khibrad la-talin sharci ah" },
  { name: "Cabdiraxmaan Nuur", role: "Sarkaalka Xaqiijinta", experience: "6+ sano oo khibrad hubinta dukumentiyada ah" },
];
    ...
    <main>
      ...
      <section id="verify" className="section verification-section">
        ...
        <div className="verification-console glass-card">
          <div className="console-heading"><div><span className="console-icon"><QrCode size={23} /></span><div><strong>Xaqiijinta Dukumentiga</strong><small>Gelitaanka Diiwaanka oo Sugan</small></div></div><span className="secure-label"><LockKeyhole size={13} />Sugan</span></div>
          <label htmlFor="reference">Lambarka Tixraaca</label><div className="verify-input"><input id="reference" value={verificationRef} onChange={(e) => { setVerificationRef(e.target.value); setVerification(null); }} placeholder="Tusaale: MNP-2026-04141" onKeyDown={(e) => e.key === "Enter" && verify()} /><button onClick={verify}>Xaqiiji</button></div>
          <input ref={scannerRef} className="visually-hidden" type="file" accept="image/*" capture="environment" onChange={() => { setVerificationRef("MNP-2026-04141"); setVerification("valid"); }} aria-label="Soo geli ama sawir Koodhka QR-ka" /><button className="scan-button" onClick={() => scannerRef.current?.click()}><ScanLine size={18} />Ka sawir Koodhka QR-ka Kamerada</button>
          <AnimatePresence mode="wait">{verification && <motion.div key={verification} className={`result-card ${verification}`} initial={{ opacity: 0, y: 12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0 }}>
            {verification === "valid" ? <><div className="result-status"><span><CheckCircle2 size={22} /></span><div><strong>Dukumentigu Waa Sax (Diiwaangashan)</strong><small>Waxaa laga xaqiijiyey diiwaanka rasmiga ah</small></div></div><div className="result-details"><dl><div><dt>Lambarka Dukumentiga</dt><dd>MNP-2026-04141</dd></div><div><dt>Magaca Milkiilaha</dt><dd>Cabdullaahi Maxamed</dd></div><div><dt>Taariikhda La Bixiyey</dt><dd>14 Luulyo 2026</dd></div><div><dt>Waqtiga Uu Ku Egyahay</dt><dd>Ma Khuseeyo</dd></div></dl><div className="qr-wrap"><QRCodeSVG value="https://marwaaznotary.com/verify/MNP-2026-04141" size={96} bgColor="transparent" fgColor="#1459ad" /><span>SHAABAD CASRI AH</span></div></div><div className="signature-line"><span>Saxiixa Rasmiga Ah</span><em>Marwaaz Notary</em><b><Stamp size={22} />LA SHAABADEEYEY</b></div></> : <div className="result-status"><span><XCircle size={22} /></span><div><strong>Tixraaca Lama Helin</strong><small>Hubi lambarka oo mar kale isku day, ama nala soo xiriir.</small></div></div>}
          </motion.div>}</AnimatePresence>{!verification && <div className="console-empty"><Upload size={22} /><span>Faahfaahinta xaqiijinta halkan ayay ka soo muuqan doontaa.</span></div>}
        </div>
      </section>

      <section id="nala-xiriir" className="section appointment-section">
        <div className="appointment-intro"><span className="eyebrow"><span />XIRIIR DEGDEG AH</span><h2>Ogow goorta uu xafiisku furan yahay, oo si toos ah nala soo xiriir.</h2><p>Xaaladda xafiisku waa mid toos ah (Live). Wac ama noo dir fariin WhatsApp ah wakhti kasta — kooxdayada ayaa si degdeg ah kuugu jawaabi doonta.</p><div className="contact-mini"><div><Phone size={19} /><span><small>Nala Soo Hadal</small><a href="tel:+252617414141">+252 617 414141</a></span></div><div><Clock3 size={19} /><span><small>Furan: Sabti – Khamiis</small><strong>7:30 subaxnimo – 7:30 fiidnimo</strong></span></div></div></div>
      ...
      <section id="team" className="section team-section"><SectionHeading eyebrow="KOOXDA XIRFADLAYAASHA" title="Khibrad, mas'uuliyad iyo adeeg kuu gaar ah" copy="Kooxdayadu waxay isku dartaa go'aan taxaddar leh, habraac nidaamsan iyo ixtiraam macmiil kasta." index="07" /><div className="team-grid">{team.map((person, index) => <motion.article className="team-card" key={person.name} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }}><div className="team-image"><span className="avatar-fallback">{initials(person.name)}</span><span className="team-badge">{index === 0 ? "Nootaayaha Guud" : "Kooxda xirfadlayaasha"}</span></div><div className="team-info"><h3>{person.name}</h3><strong>{person.role}</strong><p>{person.experience}</p><a href="#contact"><Mail size={15} />La xiriir</a></div></motion.article>)}</div></section>
